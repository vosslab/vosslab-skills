...
- Organized every tracked public and deprecated skill under metadata-defined
  workflow-role categories. Moved the skill directories with `git mv` so history
  remains traceable while invocation names and frontmatter names stay unchanged.
- Made `skills/<category>/<skill-name>/` the enforced public layout. Shared
  ...
  nesting, while keeping hidden runtime trees, gitignored, and `old-*` skills
  outside publication as appropriate.
...
- Capped every authored `SKILL.md` frontmatter description at 250 characters to preserve
  Codex's shared discovery budget, documented that repository policy, and regenerated
  `docs/SKILLS_INDEX.md` from the concise trigger descriptions.
...
- Updated the shared skill discovery used by `tools/build_skills_index.py` and
  ...
  Deprecated `old-*` skills are now excluded by shared discovery alongside hidden
  runtime trees and git-ignored skills, so every generated consumer receives the
  same publishable set.
