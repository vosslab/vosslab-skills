...
- [docset-updater/SKILL.md](../skills/docset-updater/SKILL.md): Refresh the whole repo doc set in one pass by invoking the per-doc skills in dependency order (`arch-docs`, `setup-install-usage-docs`, `readme-docs`, `agents-md-fixer`), then audit any remaining `docs/` files those skills do not own.
