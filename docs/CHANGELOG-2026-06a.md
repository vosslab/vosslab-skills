...
- Synced vendored [run_web_server.sh](../skills/management/html-game-parallel-builder/templates/run_web_server.sh)
...
- Added [PLAN_TEMPLATE_BLANK.md](../skills/planning/blueprint-plan-drafter/references/PLAN_TEMPLATE_BLANK.md):
  ...
  retained field labels are what push a manager into milestone / workstream /
  work-package structure. This is the single artifact a manager copies to start
  a plan. Reason: managers were
  ignoring the template and inventing "lanes and waves" structure instead of
  milestones and workstreams. The prior template mixed the skeleton with three
  worked examples and survey prose in one 273-line file, so "start from the
  template" had no clean copy-from target.

### Behavior or Interface Changes

- `skills/blueprint-plan-drafter/SKILL.md` Workflow step 9 now requires every
  plan to begin as a verbatim copy of `PLAN_TEMPLATE_BLANK.md`, then fill each
  field and delete unused archetype slots. Removed the prior "for small
  plans, write the plan directly" escape hatch that let managers freehand.
  Phrased positively ("Begin every plan by copying...") per the
  [REPO_STYLE.md](REPO_STYLE.md#core-philosophies) "Prompt positively"
  rule. `## Inputs to read first` lists the blank skeleton (copy this) and the
  example file (learn the shape) as separate entries.

### Fixes and Maintenance

- Renamed `references/PLAN_TEMPLATE.md` to
  [PLAN_TEMPLATE_EXAMPLE.md](../skills/planning/blueprint-plan-drafter/references/PLAN_TEMPLATE_EXAMPLE.md)
  ...
  `references/PLAN_HEADINGS.md`, and `references/plan_quality_standard.md`.
- Targeted positive-prompting pass on `SKILL.md` Mandatory constraints: flipped
  the highest-drift "Do not..." bullets to positive instructions (dependencies
  declared by ID; pair completion claims with acceptance criteria; keep
  architecture / implementation / release concerns in separate sections). Hard
  boundary negatives (do not write production code) left as-is.
- Fixed two stale markdown links in `docs/CODE_ARCHITECTURE.md` and
  `docs/FILE_STRUCTURE.md` that pointed at `tests/git_file_utils.py`; the helper
  is `tests/file_utils.py`.
- Added missing type annotations to clear `tests/test_function_typing.py`
  failures: return annotations on parametrized test functions across seven
  `tests/test_*.py` files, full annotations on `tools/list_loaded_skills.py`
  helpers, and on `strip_jwt_tree` / `redact_tree` / `normalize_messages` in
  `skills/webwork-writer/references/scripts/lint_pg_via_renderer_api.py` plus
  `main` in `contrast_calculator.py`. Full suite: 530 passed.
...
- Repaired 59 real Markdown link errors flagged by `tests/test_markdown_links.py`
  ...
  (3) redundant `..` traversal ``docs`` rewritten to `(.)` in
  ...
  `unit-test-starter`, plus literal ``URL`` template examples in
...
- [SKILL.md](../skills/planning/blueprint-plan-drafter/SKILL.md): fixed stale H1 (`Manager Make New Plan` -> `Blueprint Plan Drafter`); added "Finish the obvious" mandatory constraint citing [REPO_STYLE.md](REPO_STYLE.md) core philosophies (milestone Exit criteria must list obvious follow-ons explicitly); added parallel-plan-first design constraint (each milestone must declare 2+ independent workstreams unless inherently serial); added new `## Parallel-plan readiness checklist` section; promoted `parallel-plan` and `delegate-manager-to-subagents` in the Plan Handoff section; extended Completion Criteria to require obvious-follow-on listings and parallel-plan readiness for multi-workstream milestones.

### Additions and New Features

- New pytest [test_relative_paths.py](../tests/test_relative_paths.py) that forbids `..` markdown-link paths in `skills/**/*.md` whose targets resolve outside the skill folder. Internal `..` references (e.g. `../SKILL.md` from a `references/` subdir) stay allowed because the skill's own layout travels with it. Repo gate count is now 314.
- Renamed 6 process skills and merged `delegate-manager-to-subagents/templates/` into a single `references/role-catalog.md`:
  ...
  - `execution-manager` -> `delegate-manager-to-subagents` (verb-first; "subagents" as keyword token 4 for searchability; templates folder collapsed into [role-catalog.md](../skills/management/delegate-manager-to-subagents/references/role-catalog.md)).
...
- Added [CODE_ARCHITECTURE.md](CODE_ARCHITECTURE.md) describing the plugin's layout, major components (`skills`, `agents`, `tools`, `devel`, `tests`, [docs/](.)), data flow, indexing flow, testing gates, and extension points. Records two `Known gaps`: missing root runtime requirements manifest and a stray `tingly-foraging-mccarthy.md` plan file at the repo root.
- Added [FILE_STRUCTURE.md](FILE_STRUCTURE.md) with an ASCII top-level layout, key-subtree notes (including the `tools` vs `devel` distinction and the centrally-maintained vs repo-specific split inside [docs/](.)), generated-artifact pointers, and a `Where to add new work` section.
- Linked [CODE_ARCHITECTURE.md](CODE_ARCHITECTURE.md) and [FILE_STRUCTURE.md](FILE_STRUCTURE.md) from [README.md](../README.md) Documentation list.
- Added new skill `agents-md-fixer` ([SKILL.md](../skills/documentation/agents-md-fixer/SKILL.md)): trims and standardizes a repo's `AGENTS.md` to the 100-150 line target from [REPO_STYLE.md](REPO_STYLE.md), moving long content into `docs/*.md` and leaving links. Introduces a new `-fixer` suffix family in [SKILL_NAMING.md](SKILL_NAMING.md) for in-place trim/standardize of a single named artifact. [SKILLS_INDEX.md](SKILLS_INDEX.md) updated to 24 skills.
- Added [SKILL_PHILOSOPHY_REVIEW.md](archive/SKILL_PHILOSOPHY_REVIEW.md) applying the four canonical philosophies anchored at [REPO_STYLE.md](REPO_STYLE.md#core-philosophies) to all 23 vosslab skills, with per-skill diagnosis, conflict catalog, mode and execution-posture taxonomy, philosophy applicability matrix (with evidence IDs), and a Phase 3 verification appendix.
...
- New convention doc [SKILL_NAMING.md](SKILL_NAMING.md) with rules, suffix-family table (now including `-reader`), reserved harness tokens, and a 23-row audit table reflecting the accepted post-rename state.
...
- Extended [list_loaded_skills.py](../index_lib/list_loaded_skills.py) with a `Prefix collisions` column in default output that lists other skills sharing >=3 leading characters with each entry. `old-*` skills are exempt (deprecation marker; collisions among or with them are intentional and not interesting).
- Added `--collisions` (`-x`) flag to [list_loaded_skills.py](../index_lib/list_loaded_skills.py) that filters default output to only rows with content or prefix collisions, for fast collision-only review.

### Behavior or Interface Changes

- Rewrote [SKILL_NAMING.md](SKILL_NAMING.md) after the audit captured in `idempotent-booping-stream.md`. Retracted rule 6 (agent-form preference) and replaced with a verb-first-for-process / domain-noun-for-specialist split. Added rule 1 (first-two-tokens carry identity), rule 2 (first-3-character prefix uniqueness), rule 7 (`old-` deprecation prefix), and rule 8 (no `..` paths exiting skill folders). Updated audit table to the post-rename state (24 skills: 14 compliant, 7 accepted-rename, 3 deprecated `old-*`).
...
- Stripped 82 markdown links of the form ``path`` whose targets exit the skill folder; replaced with backticked-name references (`` `audit-code-reviewer` ``, `` `docs/REPO_STYLE.md` ``). Reason: `..` paths break when a skill is loaded outside the repo (personal overlay, marketplace plugin cache). Same-skill `..` links inside `webwork-writer/references/docs/webwork/` were preserved (they stay within the skill).
...
- Realigned `unit-test-starter` opening to [PYTEST_STYLE.md](PYTEST_STYLE.md) (prefer fewer/durable tests; delete fragile tests; route elaborate scenarios to `tests/e2e/`).
- Renamed 10 vosslab skills under `skills/` to follow [SKILL_NAMING.md](SKILL_NAMING.md). Old to new: `manager-driven-execution` to `execution-manager`; `manager-make-new-plan` to `planning-manager`; `manager-review-existing-plan` to `plan-review-manager`; `orchestrate-next-milestone` to `milestone-manager`; `pdf-skill` to `pdf-guide`; `python-code-review` to `python-reviewer`; `read-repo-rules` to `repo-rules-reader`; `review-code-changes` to `audit-code-reviewer`; `web-game-parallel-build` to `web-game-parallel-builder`. Each rename uses `git mv` or `git rm`+`git add`; frontmatter `name:` updated.
- New `## Listing loaded skills` section in [USAGE.md](USAGE.md) documents `tools/list_loaded_skills.py`.
- Skill-folder naming convention is documented in [SKILL_NAMING.md](SKILL_NAMING.md); cross-link into [REPO_STYLE.md](REPO_STYLE.md) is an upstream-vendored-doc task and is NOT applied locally.
- Regenerated [SKILLS_INDEX.md](SKILLS_INDEX.md) (23 skills, all new names).
- [list_loaded_skills.py](../index_lib/list_loaded_skills.py) default output switched from raw `name<tab>source` lines to a tabulated three-column table (Skill, Source, Prefix collisions) via the `tabulate` package; `--names-only` and `--check` modes are unchanged.
- Added `tabulate` to [pip_requirements-dev.txt](../pip_requirements-dev.txt) for the new tabulated output in [list_loaded_skills.py](../index_lib/list_loaded_skills.py).
...
- Fixed [list_loaded_skills.py](../index_lib/list_loaded_skills.py) plugin scanner: the previous walk assumed `cache/<plugin>/<version>/skills/` (3 levels) but the actual cache layout is `cache/<marketplace>/<plugin>/<version>/skills/` (4 levels), so all 16 plugin skills (14 superpowers, 1 frontend-design, 1 skill-creator) were silently missed. Replaced filesystem-walk with `~/.claude/plugins/installed_plugins.json` lookup, which gives the active install path directly and skips stale cached versions.

### Decisions and Failures

- `docs/REPO_STYLE.md` is vendored upstream; the planned `## Naming` cross-link to [SKILL_NAMING.md](SKILL_NAMING.md) cannot be applied locally and is deferred to the upstream owner.
- Codified rule 6 in [SKILL_NAMING.md](SKILL_NAMING.md): prefer agent-form suffixes (`-er`/`-or`/`-ist`); verb-form `-fix`/`-refresh` deprecated; `-docs` is a documented artifact-form exception.
- Decided mode-tag form as YAML frontmatter (top-level `mode:` and `execution:` keys), tied to evidence read from `skills/skill-writing-guide/SKILL.md`. See WP-M0 decision in [SKILL_PHILOSOPHY_REVIEW.md](archive/SKILL_PHILOSOPHY_REVIEW.md).
- Held the matrix-gate rule: skills not flagged in the matrix did not receive philosophy edits, preventing corpus-wide bloat.
- Dropped WP-X2 (separate `docs/SKILL_DEPRECATIONS.md`) by user direction during the 6-reviewer audit; deprecation candidates remain documented in the "Known gaps and deferred work" section of [SKILL_PHILOSOPHY_REVIEW.md](archive/SKILL_PHILOSOPHY_REVIEW.md). A future plan can author a deprecation doc when there is concrete deletion work.
...
- Standardized `README.md` with a concise documentation map near the top, a shorter related
  repositories section, and stale skill-count wording removed from [INSTALL.md](INSTALL.md).
...
- Expanded `README.md` with a clearer grouped skills summary and linked the generated [SKILLS_INDEX.md](SKILLS_INDEX.md) index.
