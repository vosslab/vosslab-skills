...
- Extended [list_loaded_skills.py](../tools/list_loaded_skills.py) with a `Prefix collisions` column in default output that lists other skills sharing >=3 leading characters with each entry. `old-*` skills are exempt (deprecation marker; collisions among or with them are intentional and not interesting).
- Added `--collisions` (`-x`) flag to [list_loaded_skills.py](../tools/list_loaded_skills.py) that filters default output to only rows with content or prefix collisions, for fast collision-only review.
...
- [list_loaded_skills.py](../tools/list_loaded_skills.py) default output switched from raw `name<tab>source` lines to a tabulated three-column table (Skill, Source, Prefix collisions) via the `tabulate` package; `--names-only` and `--check` modes are unchanged.
- Added `tabulate` to [pip_requirements-dev.txt](../pip_requirements-dev.txt) for the new tabulated output in [list_loaded_skills.py](../tools/list_loaded_skills.py).
