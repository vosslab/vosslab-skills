...
A continuous axis with no canonical marked points. Distinct from a scale: a scale has named anchors; a spectrum is continuous, with regions named only by convention.

## To spectrum-ify an idea
...
Spectra pair with **substitution** (slide the idea up or down the spectrum and examine each new position) and with **negation** (the opposite of a spectrum-position is its mirror across the midpoint, which is structurally distinct from "the opposite end"). They're the right organon when the idea's underlying dimension is genuinely continuous rather than categorical.
