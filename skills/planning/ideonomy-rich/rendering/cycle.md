...
A cycle is a closed loop where the endpoints connect. The structure is *recurrence*: the last state leads back to the first. Use for life cycles, OODA loops, calendars, breath, seasons.
...
```
        ...
        ╰─────────▲─────────╯               ╰─────────┬─────────╯
                  ...
                  └─────────────────────────│   ●  notice       │
                                            ...
                                            ╭───────────────────╮
                                            │   ★  return       │  ← the practice
                                            │      (re-anchor)  │     IS this step
                                            ╰─────────┬─────────╯
                                                      │
                                            (loop back to "drift")
```

Horizontal-loop form (when the cycle has many small phases):
...
- Every phase the same visual weight → no `★`, no pivot, reader doesn't know what to take
- Cycle drawn as a straight arrow chain with no loop indicator → that's a procedure, not a cycle
- Too many phases (>8) → break into a meta-cycle of phase-groups
- Bidirectional cycle drawn as one direction → lies about the structure
