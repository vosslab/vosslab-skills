#!/usr/bin/env python3
"""Extract the raw PDF text layer into page-free Markdown."""
...
import pdf_extract.raw_text


#============================================
def parse_args() -> argparse.Namespace:
	"""Parse command-line arguments for raw text-layer extraction."""
	parser = argparse.ArgumentParser(description="Extract the PDF text layer to page-free Markdown")
...
#============================================
def main() -> None:
	"""Convert one PDF from its raw text layer, or measure it without writing artifacts."""
	args = parse_args()
	...
	extracted_pages = pdf_extract.raw_text.extract_raw_text(input_path, pages)
	if args.measure_only:
		result = pdf_extract.cleanup.measure_pages(extracted_pages, "raw_text", running_head_defaults)
	else:
		result = pdf_extract.cleanup.convert_pages(
			input_path, extracted_pages, "raw_text", args.running_heads, args.page_numbers,
