...
- Brainstorming, planning, or plan creation; use `manager-make-new-plan` or `manager-review-existing-plan` instead.
...
## Lighter than review-code-changes

Quality review is ONE subagent checking vosslab rules, not the 6-reviewer audit described in [review-code-changes](../review-code-changes/SKILL.md). Defer to `/review-code-changes` only if the user later asks for a full comprehensive review.
...
- Heavyweight 6-pass reviews per task (use `/review-code-changes` for that).

## Integration with other skills

- Pairs with `manager-make-new-plan` and `manager-review-existing-plan` for plan authoring/audit.
- Lighter alternative to [review-code-changes](../review-code-changes/SKILL.md) for per-task review during execution.
