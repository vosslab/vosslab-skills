...
In this repository, create public skills at
`skills/<category>/<skill-name>/SKILL.md`. Choose the category by the skill's
primary workflow role: `orient`, `plan`, `manage`, `experts`, `docs`, or
`quality`. File every `-expert` and `-engineer` skill under `experts` so the
shared expert-skill parity checks apply. The ignored `.system/` tree is owned
by the runtime and is outside repository skill maintenance.
